import { Link } from "react-router-dom";

const Footer = () => {
  return (
    <footer className="mt-auto bg-neutral-200 rounded-lg shadow dark:bg-neutral-800">
      <div className="w-full max-w-screen-xl mx-auto p-4 md:py-8 space-y-4">
        <div className="sm:flex sm:items-center sm:justify-center">
          <ul className="flex flex-wrap items-center mb-6 text-sm font-medium text-gray-500 sm:mb-0 dark:text-gray-400">
            <li>
              <Link to={"/about"} className="hover:underline me-4 md:me-6">
                About
              </Link>
            </li>
            <li>
              <Link to={"/"} className="hover:underline me-4 md:me-6">
                Press Room
              </Link>
            </li>
            <li>
              <Link to={"/help/support"} className="hover:underline me-4 md:me-6">
                Support
              </Link>
            </li>
            <li>
              <Link to={"/help/advertising"} className="hover:underline me-4 md:me-6">
                Advertising
              </Link>
            </li>
            <li>
              <Link to={"/help/faq"} className="hover:underline me-4 md:me-6">
                FAQ
              </Link>
            </li>
            <li>
              <Link to={"/help/terms"} className="hover:underline me-4 md:me-6">
                Terms
              </Link>
            </li>
            <li>
              <Link to={"/help/privacy"} className="hover:underline me-4 md:me-6">
                Privacy
              </Link>
            </li>
            <li>
              <Link to={"/help/cookie"} className="hover:underline me-4 md:me-6">
                Cookie
              </Link>
            </li>
            <li>
              <Link to={"/help/notice-at-collection"} className="hover:underline me-4 md:me-6">
                Notice at Collection
              </Link>
            </li>
            <li>
              <Link to={"/sitemap"} className="hover:underline me-4 md:me-6">
                Sitemap
              </Link>
            </li>
          </ul>
        </div>
        <hr className="hr" />
        <span className="block text-xs text-gray-500 sm:text-center dark:text-gray-400">
          © 2024 Beehive community - All Rights Reserved.
        </span>
        <span className="block text-xs text-gray-500 sm:text-center dark:text-gray-400">
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. In fuga animi quidem repellat minus illum harum ea voluptate nemo.
        </span>
      </div>
    </footer>
  );
};

export default Footer;
